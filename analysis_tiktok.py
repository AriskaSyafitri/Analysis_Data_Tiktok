...
def train_and_evaluate(X, y):
    ...
    return model  # Hanya kembalikan model
...
def main():
    ...
    if st.session_state.section == 'EDA':
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["📊 Distribusi Interaksi", "🔗 Korelasi", "📈 Hubungan Antar Fitur", "⏰ Analisis Waktu & Kategori Musik"])
        ...
        with tab4:
            ...
            day_fig = px.bar(day_df, x=day_df.index, y=day_df.columns,
                             ...
                             labels={'x': 'Hari (0=Senin, 1=Selasa, ..., 6=Minggu)', 'value': 'Jumlah Konten', 'variable': 'Popularitas'},
                             color_discrete_sequence=['#39FF14', '#FF073A'])
            ...
            music_fig = px.bar(music_interactions, x='musicMeta.musicName', y='total_interactions',
                                title='Total Interaksi Berdasarkan Kategori Musik',
        ...
        tab1, tab2, tab3 = st.tabs(["🔮 Klasifikasi Satu Konten", "📅 Klasifikasi Banyak Konten", "✍️ Input Manual"])

        with tab1:
            text = st.text_area("Teks Konten")
            author = st.text_input("Nama Pembuat Konten")
            music = st.text_input("Nama Musik")
            duration = st.number_input("Durasi Video (detik)", min_value=1)
            waktu_jam = st.time_input("Jam Posting", datetime.strptime("12:00", "%H:%M").time())
            if st.button("Cek Popularitas Konten"):
                ...
                if result == 1:
                    ...
                    st.warning("Konten ini  **Tidak Populer**.")
        ...
        with tab3:
            ...
            for i in range(rows):
                ...
                text = st.text_area(f"Teks Konten {i+1}", key=f"text_{i}")
            ...
            if st.button("Popularitas Data Manual"):
